...
net.Receive("TTTTargetHit", function(len)
	...
	if not Bool then
		if Target:IsActive() then
			Role = L[Target:GetRoleStringRaw()]
			CLA = Target:GetRoleData().color
			CLB = Target:GetRoleData().dkcolor
		end
...
local function HUD(name, xPos, yPos, alignment, ColorA, ColorB, value, maximum)
	-- Number or String?
...
hook.Add("HUDPaint", "HUDPaint4TTTTargetHit", function()
	...
	if PreviewRendering:GetBool() then
		...
		if ply.Alive and ply:Alive() and ply:IsTerror() and not ply:IsSpec() and ply.GetRole and ply:GetRole() and ply:GetRole() == ROLES.HITMAN.index then
...
local function DefaultI()
	...
	RunConsoleCommand("ttt_target_hud_offset_x", tostring(ScrW() / 2 - 125))
...
hook.Add("TTTSettingsTabs", "TTTTarget4TTTSettingsTabsHit", function(dtabs)
	...
	if not IsValid(ply) or not ply.GetRole or ply:GetRole() ~= ROLES.HITMAN.index then return end
